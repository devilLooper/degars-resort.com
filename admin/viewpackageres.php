...
<main class="col-md-9 ml-sm-auto col-lg-10 px-md-4 py-4">
    ...
    <li class="nav-item">
        ...
        <?php $rr = mysqli_query($db, "SELECT * FROM payviaqr"); ?>
            ...
            <a class="nav-link" href="viewqrtransac.php">View Qr Tansaction
        ...
        <?php $o = mysqli_query($db, "SELECT * FROM `p-transactions`"); ?>
            ...
            <a class="nav-link active" href="viewpackageres.php">Package Reservation
    ...
    <table id="responsiveTable" class="display nowrap" style="width:100%">
        ...
        <tbody>
            <tr>
                <?php 
                $result = mysqli_query($db,"SELECT ptr.p_transac_id,ptr.transac_ref,ptr.mode_of_payment,ptr.totalrate,ptr.added_charge,ptr.add_ons,ptr.amountReceived,pres.pid,pres.resDate,pres.status,pres.checkouturl,c.pcus_id
                FROM `p-transactions` AS ptr
                INNER JOIN p_reservation AS pres ON ptr.pid = pres.pid
                INNER JOIN pcustomer AS c ON ptr.pcus_id = c.pcus_id
                ");
                
                while ($row = $result->fetch_array()) {?>
                <td><?php echo $row['transac_ref']; ?></td>
                ...
                <td>
                    <a id="" href="pkg/upexstatus.php?p_transac_id=<?php echo $row['p_transac_id']; ?>&pid=<?php echo $row['pid'];?>"onclick="return confirm('Are you sure you want to approved this reservation?');"><i class="fa-solid fa-pen-to-square"></i></a>
                    <a id="" href="pkg/cancelexstatus.php?pid=<?php echo $row['pid'];?>" onclick="return confirm('Are you sure you want to cancel this reservation?');"><i class="fa-sharp fa-solid fa-ban text-warning"></i></a>
                    <a onclick="return confirm('Are you sure you want to delete this reservation?');" class="text-danger" id="" href="pkg/deleteres.php?p_transac_id=<?php echo $row['p_transac_id'];?>&pid=<?php echo $row['pid']; ?>&pcus_id=<?php echo $row['pcus_id']; ?>"><i class="fa-solid fa-trash"></i></a>
                </td>
            </tr>
            <?php } ?>
        </tbody>
